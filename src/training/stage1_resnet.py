"""
Train the ResNet18 Model (Stage 1, 3 classes)
...
def train(args):
    ...
    full_dataset = PatchDataset(args.data, transform=None, class_names=STAGE1_CLASSES, label_remap=STAGE1_REMAP)
    n = len(full_dataset)
    ...
    for fold, (train_local, val_local) in enumerate(gkf.split(range(len(cv_idx)), cv_labels, cv_group_ids)):
        ...
        results = train_one_fold(fold, train_idx_full, val_idx_full, args.data, cfg, train_svs_fold, val_svs_fold)
    ...
    eval_ds = PatchDataset(args.data, get_eval_transform(), class_names=STAGE1_CLASSES, label_remap=STAGE1_REMAP)
    ...
    header = "            " + "  ".join(f"{name[:8]:>8}" for name in STAGE1_CLASSES)
    print(header)
    for i, name in enumerate(STAGE1_CLASSES):
        row = "  ".join(f"{cm[i, j]:>8}" for j in range(len(STAGE1_CLASSES)))
...
def main():
    parser = argparse.ArgumentParser(description="Train Resnet18 Model (Stage-1)")
