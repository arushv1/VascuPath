...
def train_one_fold(fold, train_idx, val_idx, dataset_path, cfg, train_svs, val_svs, classes):
    ...
    model = FoundationVesselDetector(num_classes=NUM_STAGE2_CLASSES, freeze_backbone=True).to(DEVICE)
...
def train(args):
    ...
    print(f"Model: Foundation Veseel Classifier (Stage 2, Stain {args.stain})")
    ...
    best_n_epochs = max(1, round(np.mean([r["best_epoch"] for r in fold_results])))

    print("Training final model on all CV data")
    print(f"Using {best_n_epochs} epochs (mean of fold best epochs: {[r['best_epoch'] for r in fold_results]})")
    ...
    final_model = FoundationVesselDetector(num_classes=NUM_STAGE2_CLASSES, freeze_backbone=True).to(DEVICE)
    criterion = torch.nn.CrossEntropyLoss()
    ...
    save_path = CHECKPOINTS_DIR / f"stage2_foundation{args.stain}_model_cv{mean_acc:.2f}_test{test_results['accuracy']:.2f}.pth"
